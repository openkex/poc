 ...
 * <p>
 * this simple variant does not distinguish left and right leave, the hashes are sorted and then concatenated.
 * the prove is simpler (it does not require left/right information)
 * the security impact of this simplification is currently unclear.
 * <p>
 * The simple variant was not implemented as option of MerkleTree to get the minimal code size.
 * (the code duplication is intended)
 */
public class MerkleTreeSimple {
     ...
     */
    public MerkleTreeSimple(List<byte[]> hashes, String algorithm) {
     ...
     */
    public List<byte[]> getProveList(int index) {
        List<byte[]> proveList = new ArrayList<>();
        ...
        while (node.getParent() != null) { // while not the root node.
            ...
            if (parent.getRightChild() == null) {
                ...
                proveList.add(parent.getRightChild().getHash());
            }
            else if (parent.getRightChild() == node) {
                proveList.add(parent.getLeftChild().getHash());
            }
            else {
    ...
    public static byte[] getNodeHash(byte[] leftHash, byte[] rightHash, MessageDigest digest) {
        ...
        digest.reset();
        // NOTE2: this is the "Simple" change of MerkleTreeSimple
        // update digest with hashes sorted by "size" (instead of left to right)
        if (ByteArrayTool.compareByteArray(leftHash, rightHash) > 0) {
     ...
     */
    public static boolean validateProveList(List<byte[]> proves, byte[] leaveHash, byte[] rootHash, MessageDigest digest) {
        byte[] hash = leaveHash; // start with leave hash.
        for (byte[] prove : proves) {
            hash = getNodeHash(hash, prove, digest);
        }
