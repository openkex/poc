...
public class MerkleTreeTest {
    ...
    public void testSmall() {
        MerkleTree tree = getTestTree(HASH_ALGORITHM, HASH_SIZE, SEED, 16, false, false);
    ...
    public void testProve() throws Exception {
        int size = 4000;
        int depth = 12;
        MerkleTree tree = getTestTree(HASH_ALGORITHM, HASH_SIZE, SEED, size, false, false);
        MessageDigest digest = MessageDigest.getInstance(HASH_ALGORITHM);
        byte[] rootHash = tree.getRootHash();
        for (int i = 0; i < size; i++) {
            List<MerkleTree.Prove> prove = tree.getProveList(i);
            Assert.assertTrue(prove.size() <= depth);
            Assert.assertTrue(MerkleTree.validateProveList(prove, tree.getLeaveHash(i), rootHash, digest));
        }
    }

    private MerkleTree getTestTree(String algorithm, int hashSize, long seed, long size, boolean sort, boolean log) {
        ...
        t.split("merkle", log);
        MerkleTree m = new MerkleTree(hashes, algorithm);
        t.stop(log);
        return m;
    }
...
//            int startSize = 10000; use this with -Xmx64m for memory limit test
    ...
    public void testBadSize() {
        ...
        try {
            new MerkleTree(hashes, HASH_ALGORITHM);
            Assert.fail();
        }
        catch (Exception e) {
            LOG.info("expected: " + e);
    ...
    public void testBitChange() {
        ...
        tree = new MerkleTree(hashes, HASH_ALGORITHM);
